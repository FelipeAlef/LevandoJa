...
type
 TClienteWebAppGlobal = class(TD2BridgeServerControllerBase)
  private
...
var
 D2BridgeServerController: TClienteWebAppGlobal;


Function Cliente: TClienteSession;
...
Function Cliente: TClienteSession;
begin
 Result:= TClienteSession(D2BridgeInstance.PrismSession.Data);
end;

constructor TClienteWebAppGlobal.Create(AOwner: TComponent);
begin
 inherited;
 {$IFDEF D2BRIDGE} 
...
procedure TClienteWebAppGlobal.OnException(Form, Sender: TObject; E: Exception; FormName, ComponentName, EventName: string; APrismSession: IPrismSession);
begin
 //Show Error Messages
 {
...
procedure TClienteWebAppGlobal.OnNewSession(const Request: TPrismHTTPRequest; Response: TPrismHTTPResponse; Session: TPrismSession);
begin
 D2BridgeInstance.PrismSession.Data := TClienteSession.Create(Session);
...
procedure TClienteWebAppGlobal.OnCloseSession(Session: TPrismSession);
begin
...
procedure TClienteWebAppGlobal.OnExpiredSession(Session: TPrismSession; var Renew: boolean);
begin
...
procedure TClienteWebAppGlobal.OnIdleSession(Session: TPrismSession; var Renew: boolean);
begin

end;

procedure TClienteWebAppGlobal.OnDisconnectSession(Session: TPrismSession);
begin

end;

procedure TClienteWebAppGlobal.OnReconnectSession(Session: TPrismSession);
begin

end;

procedure TClienteWebAppGlobal.OnSecurity(const SecEventInfo: TSecuritEventInfo);
begin
{
...
{$IFNDEF D2BRIDGE}
initialization
 D2BridgeServerController:= TClienteWebAppGlobal.Create(D2BridgeInstance.Owner);
{$ENDIF}
