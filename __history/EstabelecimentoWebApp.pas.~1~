...
type
 TEstabelecimentoWebAppGlobal = class(TD2BridgeServerControllerBase)
  private
...
var
 D2BridgeServerController: TEstabelecimentoWebAppGlobal;


Function Estabelecimento: TEstabelecimentoSession;
...
Function Estabelecimento: TEstabelecimentoSession;
begin
 Result:= TEstabelecimentoSession(D2BridgeInstance.PrismSession.Data);
end;

constructor TEstabelecimentoWebAppGlobal.Create(AOwner: TComponent);
begin
 inherited;
 {$IFDEF D2BRIDGE} 
...
procedure TEstabelecimentoWebAppGlobal.OnException(Form, Sender: TObject; E: Exception; FormName, ComponentName, EventName: string; APrismSession: IPrismSession);
begin
 //Show Error Messages
 {
...
procedure TEstabelecimentoWebAppGlobal.OnNewSession(const Request: TPrismHTTPRequest; Response: TPrismHTTPResponse; Session: TPrismSession);
begin
 D2BridgeInstance.PrismSession.Data := TEstabelecimentoSession.Create(Session);
...
procedure TEstabelecimentoWebAppGlobal.OnCloseSession(Session: TPrismSession);
begin
...
procedure TEstabelecimentoWebAppGlobal.OnExpiredSession(Session: TPrismSession; var Renew: boolean);
begin
...
procedure TEstabelecimentoWebAppGlobal.OnIdleSession(Session: TPrismSession; var Renew: boolean);
begin

end;

procedure TEstabelecimentoWebAppGlobal.OnDisconnectSession(Session: TPrismSession);
begin

end;

procedure TEstabelecimentoWebAppGlobal.OnReconnectSession(Session: TPrismSession);
begin

end;

procedure TEstabelecimentoWebAppGlobal.OnSecurity(const SecEventInfo: TSecuritEventInfo);
begin
{
...
{$IFNDEF D2BRIDGE}
initialization
 D2BridgeServerController:= TEstabelecimentoWebAppGlobal.Create(D2BridgeInstance.Owner);
{$ENDIF}
